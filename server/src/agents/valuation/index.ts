...
export async function valuationAgent(input: AgentInput): Promise<AgentOpinion> {
  ...
  let closes = await getDailyClosesUSD(input.token, 60);
  if (!closes || closes.length < 20) closes = await getDailyClosesUSD(input.token, 30);
  let stance: "BUY" | "HOLD" | "SELL" = "HOLD";
  let conf = 0.5;
  let rationaleParts: string[] = [];

  if (closes && closes.length >= 20) {
    const rets = pctReturns(closes); // daily pct returns
    const sigma = stdev(rets);       // volatility proxy
    const dd = maxDrawdown(closes);  // max drawdown fraction
    const shp = simpleSharpe(rets);  // mean/stdev (daily)
    const mu = mean(rets);
    ...
    if (analysis.score >= 70) stance = "BUY";
    else if (analysis.score <= 40) stance = "SELL";
    else stance = "HOLD";

    // Adjust stance by quant signals
    if (sharpeGood && mu > 0) {
      if (stance === "HOLD") stance = "BUY";
    }
    ...
    let agreement = 0;
    if (stance === "BUY") {
      if (analysis.score >= 70) agreement++;
      if (sharpeGood) agreement++;
      if (lowVol && mildDD) agreement++;
    } else if (stance === "SELL") {
      if (analysis.score <= 40) agreement++;
      if (sharpeBad) agreement++;
      if (highVol || deepDD) agreement++;
    } else {
    ...
    rationaleParts.push(
      `Quant: daily σ ${(sigma*100).toFixed(1)}%, max DD ${(dd*100).toFixed(0)}%, Sharpe* ${shp.toFixed(2)}, μ ${(mu*100).toFixed(2)}%.`,
      `Blend: base score ${analysis.score}/100 with quant overlays (tunable thresholds).`
    );
  } else {
    ...
    if (analysis.score >= 70) { stance = "BUY"; conf = 0.6; }
    else if (analysis.score <= 40) { stance = "SELL"; conf = 0.6; }
    else { stance = "HOLD"; conf = 0.5; }
  }
